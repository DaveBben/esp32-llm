...
#define mmap(addr, length, prot, flags, fd, offset) custom_mmap(length)
#define MAP_FAILED NULL
#define munmap(ptr, length) custom_munmap(ptr)
#define close(fd) custom_close(fd)

static const char *TAG = "LLM";
TaskHandle_t matmul_task_1 = NULL;
...
void *custom_mmap(size_t length)
{
    ...
    if (data != NULL)
    {
        memset(data, 0, length); // Simulate MAP_ANONYMOUS if needed
    }
    return data;
}
                     ...
                     int *fd, float **data, size_t *file_size)
{
    ...
    *data = malloc(*file_size);
    ...
    ESP_LOGI(TAG, "Weights pointer is %f", *weights_ptr);
    ESP_LOGI(TAG, "shared weights is %d", shared_weights);
...
void matmul_task(void *params)
{
    ...
    {   while(p->complete == 1) {}; //wait
        ...
        p->complete = 1;    // notify task is done
    }
}
...
void matmul(float *xout, float *x, float *w, int n, int d)
{
    ...
    // Dynamically allocate memory for TaskParams
    TaskParams *params1 = (TaskParams *)malloc(sizeof(TaskParams));
    TaskParams *params2 = (TaskParams *)malloc(sizeof(TaskParams));

    *params1 = (TaskParams){xout, x, w, 0, d / 2, n, d, current_task_handle, 0};
    *params2 = (TaskParams){xout, x, w, d / 2, d, n, d, current_task_handle, 0};

    if (matmul_task_1 == NULL)
    {
        xTaskCreatePinnedToCore(matmul_task, "MatMul1", 2048, params1, 1, &matmul_task_1, 0); // Run on Core 0
    }
    if (matmul_task_2 == NULL)
    {
        xTaskCreatePinnedToCore(matmul_task, "MatMul2", 2048, params2, 1, &matmul_task_2, 1); // Run on Core 1
    }
    
    // Wait for all tasks to finish
    while(params1->complete == 0 && params2->complete == 0){};
}
          ...
          char *cli_user_prompt, char *cli_system_prompt, int steps)
{
    ...
    int next = NULL;      // will store the next token in the sequence
